...
os.environ["LANGCHAIN_PROJECT"]= "GENAI Q&A Chatbot with OLLAMA"
...
def generate_response2(question,api_key,engine,temperature,max_tokens):
...
st.title("GenAI Q&A Chatbot with OpenAI and Ollama")
...
engine = st.sidebar.selectbox("Select a model", ["gpt-4o", "gpt-4-turbo","gpt-4","mistral", "llama3.1", "gemma2:2b"])
...
if engine in ["gpt-4o", "gpt-4-turbo","gpt-4"]:
    if user_input and api_key:
        response=generate_response2(user_input,api_key, engine,temperature, max_tokens)
        st.write(response)
    else:
